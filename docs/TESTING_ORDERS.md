...
Detailed cases for the test strategy: **BUY on +13% UP**, then **SL-M SELL at 2% below entry**.
...
| `dry_run` | Fake entry + fake SL-M; Telegram/log messages; **no Kite call** |
| `confirm` | Pending id → wait for `CONFIRM <id>` / CLI → then place (live or dry depending on confirm force) |
| `auto` | Immediately place entry + SL-M via Kite |
...
| TC-DRY-01 | dry_run entry + 2% SL-M | Both ok; position open; trigger rounded | `test_tc_dry_entry_and_sl_and_position` |
| TC-DRY-02 | Second BUY same symbol | Blocked “Open position” | `test_tc_dry_blocks_second_entry_while_open` |
| TC-DRY-03 | Daily cap (entry+SL count toward cap in dry_run) | Further entries blocked | `test_tc_dry_daily_cap_blocks_further_entries` |
...
| TC-LIVE-MOCK-01 | `auto` with fake Kite | MARKET BUY then SL-M SELL params correct | `test_tc_live_mock_auto_places_market_then_slm` |
...
**Expect:** DEMO13 ramps past +13%; logs show `DRY RUN: would BUY` and `DRY RUN: would place SL-M`; `.nse_alert/orders.json` has history + open position.
...
| TC-HAND-03 | Confirm a pending with **qty=1** on a cheap liquid name | Kite shows MARKET BUY + SL-M SELL; app records position | **Real money** |
...
| TC-HAND-09 | SL trigger behaviour | If price falls 2%, SL-M triggers on exchange (observe in Kite) — app does not manage fill beyond placement | Market |
...
1. In **dry_run**, entry **and** SL each increment `placed_count` → a cap of `3` allows only one full entry+SL plus limited room.  
2. Test strategy is **BUY + downside SL only**; DOWN alerts do not trade when `TRADE_SIDES=up`.  
3. Confirm path forces place mode `auto` when executor is not `dry_run` (see `_confirm_pending` in `cli.py`).  
4. Login/token flow is unchanged by order modes — refresh token daily before live tests.
